...
class PitchParameters:
    ...
    # オプション: 絶対座標系の角速度 (rad/s) を直接指定．いずれかが None でなければ
    # 上の backspin_rpm/sidespin_rpm/wg_rpm は無視され，simulate() はこの ω
    # ベクトルを直接使う．HawkEye / optical mocap / IMU 等で ω を直接測って
    # いる場合に使用．3 つ全てを同時に指定すること．
...
class BallTrajectorySimulator2:
    ...
    def simulate(self, pitch: Optional[PitchParameters] = None,
        ...
        # 角速度ベクトル wx, wy, wz とそのスカラー量 (spin_total, omega_total)
        # 分岐:
        #   (A) PitchParameters.wx_rad_s が指定されていれば，絶対座標系の ω を
        #       直接使用（HawkEye / optical mocap / IMU 等）．
        #   (B) 未指定なら BSG (backspin/sidespin/wg) から Nathan 式で構成．
        if pitch.wx_rad_s is not None:
            if pitch.wy_rad_s is None or pitch.wz_rad_s is None:
                raise ValueError(
                    "PitchParameters: wx_rad_s, wy_rad_s, wz_rad_s must all be "
                    "set together (or all None)"
                )
            wx = pitch.wx_rad_s
            wy = pitch.wy_rad_s
            wz = pitch.wz_rad_s
            ...
            # 横スピン大きさ = |ω - (ω·v̂)v̂|（リリース速度に直交する成分）
            omega_dot_v_hat = (wx * v0x + wy * v0y + wz * v0z) / v0
            omega_total = math.sqrt(
                max(omega_norm_rads * omega_norm_rads - omega_dot_v_hat * omega_dot_v_hat, 0.0)
            ) + 0.001
        else:
            # スピンベクトル（rad/s）
            ...
            # スピン角速度ベクトル
            # この (B, S, W) → (wx, wy, wz) 行列は Nathan の Excel 公開版
            # (TrajectoryCalculator-new-3D-May2021.xlsx, cells D10-D12) と
            # 完全に一致する仕様．
            # 注意: statcast_spin_to_bsg が用いる eb/es/eg 直交基底とは ωy 成分で異なる:
            #     Δωy(sim - eb) = 2 (B sin φ - S sin θ cos φ) · RPM_TO_RADS
            # このため pure backspin (S=W=0) は φ≠0 で v̂_release と完全直交ではない:
            #     ω · v̂_release = -B sin(2φ) cos θ · RPM_TO_RADS  （小さい gyro 汚染）
            # 影響は φ が小さい範囲（典型 2-3°）では plate 位置で数 mm 程度．
